package me.brokeski.blockbank.menu.menusteller.checkings;

import me.brokeski.blockbank.BlockBank;
import me.brokeski.blockbank.database.AccountQueries;
import me.brokeski.blockbank.menu.menusteller.checkings.CheckingAccountMenu;
...
public class WireSelectionMenu extends PaginatedMenu {
    ...
    public void loopCode(Object o) {
        ...
        itemMeta.getPersistentDataContainer().set(new NamespacedKey(BlockBank.getPlugin(), "uuid"), PersistentDataType.STRING, p.getUniqueId().toString());
    ...
    public void handleMenu(InventoryClickEvent inventoryClickEvent) throws MenuManagerNotSetupException, MenuManagerException {
        ...
        if (inventoryClickEvent.getCurrentItem().getType() == Material.PLAYER_HEAD){
            wireTransferConversation(p, Bukkit.getPlayer(UUID.fromString(inventoryClickEvent.getCurrentItem().getItemMeta().getPersistentDataContainer().get(new NamespacedKey(BlockBank.getPlugin(), "uuid"), PersistentDataType.STRING))));
    ...
    private void wireTransferConversation(Player player, Player target) {
        ...
        new ConversationFactory(BlockBank.getPlugin())
                .withModality(false)
                .withTimeout(15)
